...
import Datatable from './customerGroupDataTable';
...
export class CustomerGroup extends Component {
    ...
    componentWillMount(){
        apiservice.getAll('customersgroups/customers_groups_listing')
            .then((val)=>{
                console.log('val:',val);
                if(val){
                    this.setState({custGroups: val.result})
                }else{
    ...
    render() {
        ...
        if (!this.state.custGroups) return (<Loader />);
        return (
            <Fragment>
                <Breadcrumb title="Customer Group" parent="Customers" />
